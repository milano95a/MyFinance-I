...
extension ManagerExpense {
    ...
    func getProductInflation(_ expense: Expense) -> [ProductInflation] {
        var items = [ProductInflation]()
        let productEntries = Expense.fetchRequest(.all).filter("name = %@", expense.name)
        var productsByYear = [Int: [Expense]]()
        
        for productEntry in productEntries {
            if let _ = productsByYear[productEntry.date.year] {
                productsByYear[productEntry.date.year]!.append(productEntry)
            } else {
                productsByYear[productEntry.date.year] = [productEntry]
            }
        }
        
        for (year, products) in productsByYear {
            let sortedProducts = products.sorted { $0.date > $1.date }
            let productInflation = ProductInflation(
                year: year,
                productName: products[0].name,
                firstMonthMeanPrice: sortedProducts.last!.price,
                lastMonthMeanPrice: sortedProducts.first!.price)
            items.append(productInflation)
        }
